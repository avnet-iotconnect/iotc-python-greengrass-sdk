...
sudo -E /greengrass/v2/bin/greengrass-cli \
  component details --name "com.avnet.example.IotConnectSdkBasicDemo" | true

sudo -E /greengrass/v2/bin/greengrass-cli \
  deployment create \
  --remove "com.avnet.example.IotConnectSdkBasicDemo" | true

sudo -E /greengrass/v2/bin/greengrass-cli \
  deployment create \
  --recipeDir greengrass-build/recipes \
  --artifactDir greengrass-build/artifacts \
  --merge "com.avnet.example.IotConnectSdkBasicDemo=0.0.5"
...
IOTC_CONFIG=$(cat <<EOF
{
  "com.avnet.example.IotConnectSdkBasicDemo": {
     "MERGE": {
      "IOTC_CPID": "${IOTC_CPID}",
      "IOTC_ENV": "${IOTC_ENV}"
    }
...
--merge "com.avnet.example.IotConnectSdkBasicDemo=0.0.1" \
